/*	AUTHOR `NOMDEPLUME` ( benjamin@NOMDEPLUME.com / https://blog.NOMDEPLUME.com )
			...
			if(status == RPC_S_OK)
			{
				...
				if(status == RPC_S_OK)
				{
					...
					if(status == RPC_S_OK)
					{
						...
						status = RpcEpRegister(drsuapi_v4_0_s_ifspec, vector, NULL, (RPC_WSTR) straykatz L" Ho, hey! I\'m a DC :)");
